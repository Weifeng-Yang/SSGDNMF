...
function [var,loss,timerun]=L21SSGDNMF(var,ngmar,maxiteropt,stopindex,r,btmax,rho,alphat,lamda)
...
for j=1:num
    ...
    if(j<num)
        var{j}=PROXL21(V,lamda(j),1/(L(j)*r));
    else
        var{j}=PROXn1(V);
    end
    vv{j}=var{j};
...
function loss=computeloss(ngmar,var,alpha,Lapk,lamda)
    ...
    for i=1:length(var)
    loss=loss+lamda(i)*sum(sqrt(sum(var{i}.^2, 2)));
    end
    loss=loss+alpha(end)/2*trace(var{end}*Lapk{end}*var{end}');
end
